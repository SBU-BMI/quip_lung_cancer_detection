...
parser.add_argument('--neg_sampling', default=1, type=float, help='multiply of negative class in sampling')
...
print(args.weighted, args.sampler)
args.net_type = os.path.basename(__file__).split('.')[0] + '_netDepth-' + str(args.net_depth) +\
                '_weighted-' + str(args.weighted) + '_sampler-' + str(args.sampler) + '_APS-' + str(args.APS) +\
                '_negSampling-' + str(args.neg_sampling)
...
mean = [ 0.8301,  0.6600,  0.8054]  # for lung john
...
source = '/data10/shared/hanle/extract_lung_Rutger_seer_john/'
train_seer_fol = source + 'patches_lung_seer_john'
train_wenjin_fol = '/data2/patches_lung_seer_train_5000_66wsi_400'
val_fol = source + 'patches_lung_seer_john_validation'

img_trains = [f for f in glob.glob(os.path.join(train_seer_fol, '*png'))] +\
